    ...
    tf_broadcaster(this)
{
    ...
    declare_parameter("detector.refine", bool(td->refine_edges), descr("snap to strong gradients"));
    declare_parameter("detector.sharpening", td->decode_sharpening, descr("sharpening of decoded images"));
    declare_parameter("detector.debug", bool(td->debug), descr("write additional debugging images to working directory"));
